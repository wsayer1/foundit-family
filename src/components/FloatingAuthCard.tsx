...
import { MapPin, Eye, EyeOff, Loader2, Mail, ChevronDown } from 'lucide-react';
...
export function FloatingAuthCard({ onSuccess }: FloatingAuthCardProps) {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      targetPosition.current = {
        x: normalizedX * maxX * 0.4,
        y: normalizedY * maxY * 0.4
      };
    };
  ...
  return (
    <div
      ref={cardRef}
      className="w-[420px] max-w-[calc(100vw-32px)] transition-transform duration-100 ease-out"
      style={{
    ...
    >
      <div className="bg-white/95 dark:bg-stone-900/95 backdrop-blur-xl rounded-3xl shadow-2xl shadow-black/30 p-8 border border-white/20 dark:border-stone-700/50">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 text-stone-600 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-200 py-3 rounded-xl font-medium transition-colors"
        >
          <Mail size={18} />
